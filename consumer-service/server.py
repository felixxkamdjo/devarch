...
def connect_with_retry(max_attempts=10, delay=3):
    ...
    params = pika.ConnectionParameters(
        host=RABBITMQ_HOST, credentials=credentials, heartbeat=60
    )

    for attempt in range(1, max_attempts + 1):
        try:
            print(
                f"[consumer] Connecting to RabbitMQ ({attempt}/{max_attempts})…",
                flush=True,
            )
...
def run():
    ...
    for queue in QUEUES:
        ...
        channel.basic_consume(queue=queue, on_message_callback=HANDLERS[queue])
