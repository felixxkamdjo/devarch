...
def route_request(handler):
    ...
    #  /articles 
    if path == "/articles":
        if method == "GET":  return list_articles_handler(handler)
        if method == "POST": return create_article_handler(handler)
    ...
    if path.startswith("/articles/"):
        ...
        if len(parts) == 3:
            ...
            if method == "GET":    return get_article_handler(handler, article_id)
            if method == "PUT":    return update_article_handler(handler, article_id)
            if method == "DELETE": return delete_article_handler(handler, article_id)
        ...
        if len(parts) == 4 and parts[3] == "comments":
            ...
            if article_id is None:
                return send_error(handler, "Invalid article ID", 400)
            if method == "GET":  return list_comments_handler(handler, article_id)
            if method == "POST": return create_comment_handler(handler, article_id)
    ...
    if path.startswith("/comments/"):
        ...
        if method == "PUT":    return update_comment_handler(handler, comment_id)
        if method == "DELETE": return delete_comment_handler(handler, comment_id)

    #  /categories 
    if path == "/categories":
        if method == "GET":  return list_categories_handler(handler)
        if method == "POST": return create_category_handler(handler)

    if path.startswith("/categories/"):
        category_id = extract_id(path)
        if category_id is None:
            return send_error(handler, "Invalid category ID", 400)
        if method == "GET": return get_category_handler(handler, category_id)
