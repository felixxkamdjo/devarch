...
from utils import parse_json_body, send_json, send_error, read_bearer_token
...
def register_handler(handler):
    ...
    try:
        ...
        auth_result = login_user(email=body.get("email"), password=body.get("password"))
...
def login_handler(handler):
    try:
        ...
        send_json(
            handler,
            {"success": True, "message": "Login successful.", "data": auth_result},
            200,
        )
...
def current_user_handler(handler):
    try:
        ...
        send_json(handler, {"success": True, "user": user}, 200)
